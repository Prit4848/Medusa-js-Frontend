  ...
  if (count === 0) {
    return (
      <div className="flex items-center gap-3 mb-8 lg:mb-10">
        ...
        <span className="text-gray-400 text-base lg:text-lg">No reviews yet</span>
      </div>
    )
  }
...
export default function ProductDetails({
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (!variantId) return toast.error("No variant found for this product");
      if (variant.manage_inventory && (variant.inventory_quantity ?? 0) < quantity)
        return toast.error("This product is out of stock");
      if (!variant.price || variant.price <= 0)
        return toast.error("This product is not priced for the selected region");

      const result = await addToCart({ variantId, quantity, countryCode: DEFAULT_COUNTRY_CODE });
      if (!result.success) return toast.error(result.error || "Unable to add item to cart");
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      else toast.error(result.error || "Unable to process request");
    } catch {
    ...
    <>
      <div className="max-w-[1280px] mx-auto px-4 lg:px-6 py-8 lg:py-12">
        ...
        <div className="border-b border-gray-200 pb-6 lg:pb-8 mb-8 lg:mb-10 text-[13px] lg:text-[15px] text-gray-400 flex flex-wrap items-center gap-2">
          <Link href="/shop" className="hover:text-[#c87a4c]">Products</Link>
          <span>&gt;</span>
          <Link href={`/categories/${product.category_handle}`} className="hover:text-[#c87a4c]">
            {product.category}
          </Link>
          <span>&gt;</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20">
          ...
          {/* Right Content */}
          <div>
            <p className="text-[12px] text-gray-500 mb-4 lg:mb-8">{product.category}</p>
            ...
            {/* Dynamic stars */}
            <StarDisplay average={averageRating} count={reviewCount} />
            ...
            <div className="flex flex-col sm:flex-row gap-10 lg:gap-24 mb-10 lg:mb-14">
              <div>
                <p className="font-bold text-[12px] mb-4">QUANTITY</p>
              ...
              <div>
                <p className="font-bold text-[12px] mb-4">PRICE</p>
                <p className="font-semibold text-[22px]">{formattedPrice}</p>
              </div>
            </div>
